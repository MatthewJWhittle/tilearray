...
from tilearray.tiles import create_tile_grid
...
def _bbox(min_x: float, min_y: float, max_x: float, max_y: float) -> BoundingBox:
    return BoundingBox(min_x=min_x, min_y=min_y, max_x=max_x, max_y=max_y, crs=CRS.EPSG_4326)


def test_create_tile_grid_aligned_bbox() -> None:
    bbox = _bbox(0, 0, 100, 100)
    grid = create_tile_grid(bbox, tile_size=(10, 10), origin=(0, 0), resolution=(1.0, 1.0))
...
def test_create_tile_grid_respects_origin_offset() -> None:
    ...
    grid = create_tile_grid(bbox, tile_size=(10, 10), origin=(100, 200), resolution=(1.0, 1.0))
...
def test_create_tile_grid_uses_anisotropic_resolution() -> None:
    ...
    grid = create_tile_grid(bbox, tile_size=(10, 10), origin=(0, 0), resolution=(2.0, 1.0))
...
def test_create_tile_grid_handles_negative_coordinates() -> None:
    ...
    grid = create_tile_grid(bbox, tile_size=(10, 10), origin=(-50, -50), resolution=(1.0, 1.0))
