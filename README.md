...
- Fetches tiles through a shared HTTP engine with bounded concurrency, a **pressure classifier** (status + body signatures → retry / AIMD / circuit breaker), optional rate limits, and **adaptive concurrency (AIMD)** where enabled — plus thin **fetch presets** for well-known public hosts (OpenStreetMap Foundation tiles, Environment Agency WCS). Failed tiles after retries raise `NetworkError` rather than leaving silent NaN holes.
...
For multi-tile mosaics, prefer **lazy** `create_array` and **`compute_with_policy`** instead of `load_array` (which eagerly fetches on return). Plain `.compute()` uses Dask's default thread pool (~CPU cores), which can leave AIMD fetch headroom unused when `max_concurrent_requests` is higher (e.g. 10 on EA WCS).
...
One shared in-flight gate covers the whole client: Adaptive Increase/Multiplicative Decrease (AIMD) caps how many tile downloads run at once, and Dask processes tiles as they arrive. Mosaic size only lengthens the queue — it does not raise concurrency. AIMD only searches between the floor and the ceiling — the ceiling is the host's don't-cross line, not a target. Hosts still differ: Environment Agency (EA) Lidar behind Azure App Gateway often rejects a sustained burst at 32, so EA's safe sustained ceiling is about 10, not "32 is always fine."
...
- **`WCSConfig.for_ea_dsp()`** — [Environment Agency Data Service Platform (EA DSP)](https://environment.data.gov.uk/) WCS. Uses **AIMD** (*additive increase, multiplicative decrease*): starts at 8 in-flight (warm), ramps while the host is happy (ceiling **10** — a `FetchPolicy` knob, not a special code path; lowered from 32 after county-scale cold-mosaic 403 storms on Azure App Gateway), remembers the last good per-host limit in-process for later mosaics, and backs off on classified gateway pressure (403/408/429/5xx, OGC transient 404 bodies, ×0.75, floor 4). A **pressure circuit breaker** (6+ breaker-classified events such as gateway 403 or 429 within 5 s → freeze at floor for 15 s) prevents retry amplification during WAF storms. No fixed requests-per-second cap. Four retries, 60 s timeout; `Retry-After` is still honoured by `TileFetcher`. Failed tiles after retries raise `NetworkError` and abort sibling in-flight tile fetches (Dask does not cancel other graph tasks — already-started HTTP may still finish).
...
- One shared path: `classify_response` → retryable pressure → AIMD decrease / optional circuit breaker. Built-in signatures cover gateway **403, 408, 429, 500, 502, 503, 504** and OGC transient **404** bodies (`ExceptionReport`, `InvalidParameterValue`, `SUBSETTINGCRS`); register custom signatures via `PressureClassifier.register` when a new dialect appears.
...
- **County-scale cold mosaics (≥~256 tiles):** keep `max_concurrent_requests` ≤ the EA preset ceiling (**10**). Live Aire valley (~1015 tiles) at ceiling 32 triggered Azure App Gateway 403 storms (~888 requests, ~345 retries, hard fail ~tile 539); the same job at ceiling 8 completed (~88 retries, ~315 s). Small mosaics (≤64 tiles) tolerate higher ceilings, but the preset now defaults to the safer cap.

These are example policies for testing host quirks, not a product catalogue. For custom endpoints, use `from_url` and tune `FetchPolicy` / `ServiceConfig` fields (`adaptive_concurrency`, `initial_concurrent_requests`, `min_concurrent_requests`, `max_concurrent_requests`, `forbidden_circuit_breaker`, `rate_limit_per_second`, and so on). The EA preset ceiling of **10** is a safety max AIMD tunes under (`max_concurrent_requests`) — it only helps if Dask can run that many tile fetches in parallel. **Multi-tile lazy mosaics:** prefer `compute_with_policy()` over plain `.compute()` so Dask thread count matches the fetch ceiling instead of defaulting to CPU count (`create_array(..., compute=True)` sets workers automatically; otherwise use `compute_with_policy` or size manually via `compute_thread_pool_size(fetch_policy)`). Inspect live backoff via `TileFetcher.stats.current_limit`, `pressure_event_count`, and `circuit_breaker_trips`.

Offline before/after bench: `uv run python scripts/bench_fetch_engine.py` (results in `benchmarks/fetch_engine_bench_results.txt`). On a live Skipton-scale EA mosaic (~16 tiles), warm start often lands ~5–10 s (EA jitter); the prior AIMD preset (start=2) was ~14 s, legacy unbounded ~10–13 s when healthy, and the old polite fixed cap of 2 at 1 req/s ~25 s — stability under 429 still matters. On a live 64-tile (~5 km / 1000×1000) EA mosaic, ceiling 10 with Dask workers aligned via `compute_thread_pool_size` is typically sufficient; do not raise `max_concurrent_requests` above the preset ceiling for county-scale cold mosaics.
...
**Decode** (`tilearray.decode`) — unwrap response bytes → read (GeoTIFF / JPEG / PNG) → apply band policy → `float32`. JPEG and PNG use **`preserve`**: colour mosaics stay `(y, x, band)` (e.g. RGB is 3 bands). GeoTIFF elevation uses **`first_band`** (single `(y, x)` surface). **`sniff_effective_content_type`** trusts bytes over headers (multipart boundaries, TIFF magic); **`unwrap_multipart`** strips `multipart/related` WCS payloads (GML + TIFF) to the GeoTIFF part; raw GeoTIFF bytes pass through unchanged. Disk cache keeps a `.meta` sidecar but warm reads sniff the body when headers lie. Proven on real endpoints: Environment Agency Lidar WCS (live integration) and USGS 3DEP ArcGIS ImageServer WCS (second dialect; offline contract cassettes). You can still pass a custom `tile_decoder` to `create_array` when you need different behaviour.
